...
const (
	maxMetadataSize   = 1 + binary.MaxVarintLen64*2 + binary.MaxVarintLen32 // 数据类型(1) + 过期时间,版本号(binary.MaxVarintLen64 * 2) + 数据量(size)
	extraListMetaSize = binary.MaxVarintLen64 * 2                           // head,tail(binary.MaxVarintLen64 * 2)

	initialListMark = math.MaxInt64 / 2
)
...
func (zk *zsetInternalKey) encodeWithScore() []byte {
	sorceBuf := utils.Float64ToBytes(zk.score)
	buf := make([]byte, len(zk.key)+len(zk.member)+len(sorceBuf)+8+4)
	...
	// score
	copy(buf[index:index+len(sorceBuf)], sorceBuf)
	index += len(sorceBuf)
