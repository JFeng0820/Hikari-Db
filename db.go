...
func (db *DB) Stat() *Stat {
	...
	var dataFile = uint(len(db.olderFiles))
	if db.activeFile != nil {
		dataFile += 1
	}
...
// Fold 获取所有数据，并执行用户指定的操作,函数返回 false 时终止遍历
...
func (db *DB) appendLogRecord(logRecord *data.LogRecord) (*data.LogRecordPos, error) {
	...
	if !needSync && db.options.BytesParSync > 0 && db.bytesWrite >= db.options.BytesParSync {
		needSync = true
	}
	if needSync {
		if err := db.activeFile.Sync(); err != nil {
...
func (db *DB) loadDataFiles() error {
	...
	for i, fid := range fileIds {
		...
		// 如果 i == len(fileIds) -1 说明这是最后一个文件，id是最大的，说明是当前活跃文件
		if i == len(fileIds)-1 {
			db.activeFile = dataFile
		} else {
			// 说明是旧数据文件
